#!/usr/bin/python
"""
BLE Observer for Raspberry Pi 3  (Reference Code)
...
# Sensor will be inactive state if there is no advertising data receive in this timeout period.
...
FLUENTD_TAG = "xxx"
FLUENTD_ADDRESS = "xxx.xxx.xxx.xxx" ## IP Address of fluentd host 
FLUENTD_PORT = 24224

## InfluxDB
INFLUXDB_ADDRESS = "xxx.xxx.xxx.xxx" ## IP Address of Cloud Server
INFLUXDB_PORT = 8086
...
class SensorBeacon:
    ...
    def upload(self):
        # This is the function where we can process received sensor data. (ie. forwarding to fluentd data collector, uploading to cloud DB, saving to CSV...)
        if (USE_FLUENTD == True):
...
def fluentdInit():
    ...
    r = requests.get('http://164.70.7.153:8086/query', params=v)
    #print r.status_code 
...
# def resetBle():
    ...
    if (sensorBeaconList == None):
        ...
        #     print sensors.tick_last_update
